...
void AStatsManager::BeginPlay()
{
	...
	//GEngine->AddOnScreenDebugMessage(1, 9999, FColor::White, FString("Index: ") += FString::FromInt(StringArray.Num()));
}
...
TArray<FStatStruct> AStatsManager::LoadStatsToArray(const TArray<FString> ParsedStringArray, const int32 numberOfDataPoints)
{
	...
	TArray<FStatStruct> StatArray;
	for (int i = 0; i < totalStructAmount; i++)
	{
		
		FStatStruct newStat = FStatStruct(
